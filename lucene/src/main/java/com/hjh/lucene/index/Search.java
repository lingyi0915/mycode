...
public class Search {
    ...
    public void setUp() throws Exception {
        dir= FSDirectory.open(Paths.get("D:\\workspace\\索引目录"));
        reader= DirectoryReader.open(dir);
        is=new IndexSearcher(reader);
    }
    ...
    public void testTermQuery2(String field,String value)throws Exception{
        Term t=new Term(field,value);
        Query query=new TermQuery(t);
        TopDocs hits=is.search(query, 113);
        System.out.println("匹配 '"+field+","+value+"'，总共查询到"+hits.totalHits+"个文档");
        for(ScoreDoc scoreDoc:hits.scoreDocs){
            Document doc=is.doc(scoreDoc.doc);
    ...
    public static void main(String[] args) {
        ...
        Search search = new Search(indexDir);

        try {
            search.testTermQuery2("dt","20180901");
//            search.testTermQuery2("filetype","log");
        } catch (Exception e) {
            e.printStackTrace();
        }
