...
/**
 * {@link DocumentsWriterDeleteQueue} is a non-blocking linked pending deletes queue.
 * {@link DocumentsWriterDeleteQueue} 是一个非阻塞连接的挂起删除队列。
 * In contrast to other queue implementation we only maintain the tail of the queue.
 * 与其他队列实现相比，我们只维护队列的尾部。
 *  A delete queue is always used in a context of a set of DWPTs and a global delete pool.
 *  删除队列总是在一组DWPTs和一个全局删除池的上下文中使用。
 *  Each of the DWPT and the global pool need to maintain their 'own' head of the queue
 *  DWPT和全局池中的每个都需要维护自己的队列头
 *  (as a DeleteSlice instance per {@link DocumentsWriterPerThread}).
 *  (作为DeleteSlice实例 每{@link DocumentsWriterPerThread})
 ...
 * DWPT和全局池之间的区别在于，DWPT在添加了第一个文档后就开始维护head，
 * 因为对于它的段，私有删除只有在该文档之后的删除是相关的。
 * 一旦创建了这个实例，全局池就开始维护head，方法是将sentinel实例作为初始head。
 * <p>
