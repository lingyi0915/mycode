...
final class DocumentsWriterPerThreadPool {
  ...
  final static class ThreadState extends ReentrantLock {
    DocumentsWriterPerThread dwpt;// dwpt线程
    // TODO this should really be part of DocumentsWriterFlushControl
    // write access guarded by DocumentsWriterFlushControl 由DocumentsWriterFlushControl保护写访问
    volatile boolean flushPending = false;
    // TODO this should really be part of DocumentsWriterFlushControl
    // write access guarded by DocumentsWriterFlushControl 由DocumentsWriterFlushControl保护写访问
    long bytesUsed = 0;

    // set by DocumentsWriter after each indexing op finishes 最后一次seqNo
    ...
    boolean isInitialized() {
      assert this.isHeldByCurrentThread();
      return dwpt != null;// dwpt不为null即为初始化过 创建的时候为null，未初始化
    }
  ...
  private synchronized ThreadState newThreadState() {
    ...
    threadState.lock(); // 锁定，这样其他人就不会得到这个线程状态
    threadStates.add(threadState);// 将当前的DWPT添加到List中
    return threadState;
  }
  ...
  ThreadState getAndLock(Thread requestingThread, DocumentsWriter documentsWriter) {
    ...
    synchronized (this) {
      if (freeList.isEmpty()) {// 如果没有空闲的DWPT 创建新的dwpt
        // ThreadState is already locked before return by this method:
        return newThreadState();// 创建新的Thread State 线程情景
      } else {
