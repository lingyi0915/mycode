...
final class DocumentsWriterFlushControl implements Accountable {
  ...
  ThreadState obtainAndLock() {
    ...
    final ThreadState perThread = perThreadPool.getAndLock(Thread
        .currentThread(), documentsWriter);//当前的dwpt封装类 ThreadState
    boolean success = false;
    try {// 如果dwpt初始化过，并且，dwpt的删除队列 != 当前的删除队列
      if (perThread.isInitialized() && perThread.dwpt.deleteQueue != documentsWriter.deleteQueue) {
        // There is a flush-all in process and this DWPT is
        // now stale -- enroll it for flush and try for
        // another DWPT:
        addFlushableState(perThread);
      }// 操作执行成功，或者直接跳过操作! 因为DWPT为null
      success = true;
      // 只要返回线程状态，即使在所有情况下都是齐头并进，因为我们已经持有锁
      return perThread;
    } finally {
  ...
  /**
   * Returns <code>true</code> iff stalled 失速的
   */
